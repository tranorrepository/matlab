function sectionPoints = read_kml(fileName)
...
fclose(fid);
[a,b]=size(lat);
lat=reshape(lat,max(a,b),min(a,b));%latitude weidu
lon=reshape(lon,max(a,b),min(a,b));%longitude jingdu
z=reshape(z,max(a,b),min(a,b));%height

sectionPoints = [lon lat z];
