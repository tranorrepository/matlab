...
for ii = 1:num_of_Tcrossing
    for mm = 1:num_of_angles
        for nn = 1:num_of_lines
            figure(1000); subplot(211);
            plot(merged_dir_lines(ii).lines(mm).line(nn).x, ...
                merged_dir_lines(ii).lines(mm).line(nn).y, color(nn));
            hold on; axis equal;
...
for ll = 1:num_of_tmp_lines
    ...
    for pp = 1:num_of_parts
        ...
        num_of_pnts = length(merged_dir_lines(ii).lines(mm).line(nn).x);
        pnt_ind = num_of_added_pnts + 1:num_of_added_pnts + num_of_pnts;
        
        if ll <= (num_of_tmp_lines - 2)
            if mod(pp, 2) == 1
                all_tmp_lines(ll).x(pnt_ind) = ...
                    flip(merged_dir_lines(ii).lines(mm).line(nn).x);
                all_tmp_lines(ll).y(pnt_ind) = ...
                    flip(merged_dir_lines(ii).lines(mm).line(nn).y);
            else
                all_tmp_lines(ll).x(pnt_ind) = ...
                    merged_dir_lines(ii).lines(mm).line(nn).x;
                all_tmp_lines(ll).y(pnt_ind) = ...
                    merged_dir_lines(ii).lines(mm).line(nn).y;
            end
        else
            if mod(pp, 2) == 0
                all_tmp_lines(ll).x(pnt_ind) = ...
...
for ii = 1:items
    ...
    ddx = linspace(dx, 0, pnts(ii));
    ddy = linspace(dy, 0, pnts(ii));
    ...
    all_output_lines(st_st(ii, 1)).x(1:pnts(ii)) = ...
        all_output_lines(st_st(ii, 1)).x(1:pnts(ii)) + ddx;
    all_output_lines(st_st(ii, 1)).y(1:pnts(ii)) = ...
        all_output_lines(st_st(ii, 1)).y(1:pnts(ii)) + ddy;
