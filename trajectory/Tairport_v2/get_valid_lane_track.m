...
%% interpolate left and right lines
ROAD_WIDTH = 5;
...
for ii = 1:num_of_valid_gps_tracks
    ...
    for jj = 1:num_of_pnts - 1
        ...
        for kk = jj+1 : num_of_pnts
            if 0.005 <= abs(valid_gps_track_data(ii).y(kk) - ...
                    valid_gps_track_data(ii).y(jj))
                break;
            end
        end
        theta = atan2(valid_gps_track_data(ii).y(kk) - ...
            valid_gps_track_data(ii).y(jj), ...
            valid_gps_track_data(ii).x(kk) - ...
            valid_gps_track_data(ii).x(jj));
        
        left.x(jj) = valid_gps_track_data(ii).x(jj) - ...
            0.5 * ROAD_WIDTH * sin(theta);
        left.y(jj) = valid_gps_track_data(ii).y(jj) + ...
            0.5 * ROAD_WIDTH * cos(theta);
        
        right.x(jj) = valid_gps_track_data(ii).x(jj) + ...
            0.5 * ROAD_WIDTH * sin(theta);
        right.y(jj) = valid_gps_track_data(ii).y(jj) - ...
            0.5 * ROAD_WIDTH * cos(theta);
    end
    
    % for last point
    theta = atan2(valid_gps_track_data(ii).y(end) - ...
        valid_gps_track_data(ii).y(end - 1), ...
        valid_gps_track_data(ii).x(end) - ...
        valid_gps_track_data(ii).x(end - 1));
    left.x(end) = valid_gps_track_data(ii).x(end) - ...
        0.5 * ROAD_WIDTH * sin(theta);
    left.y(end) = valid_gps_track_data(ii).y(end) + ...
        0.5 * ROAD_WIDTH * cos(theta);
    right.x(end) = valid_gps_track_data(ii).x(end) + ...
        0.5 * ROAD_WIDTH * sin(theta);
    right.y(end) = valid_gps_track_data(ii).y(end) - ...
        0.5 * ROAD_WIDTH * cos(theta);
    
    valid_lane_data(ii).left = left;
    valid_lane_data(ii).right = right;
end
...
for ii = 1:num_of_valid_gps_tracks
    plot(valid_gps_track_data(ii).x, valid_gps_track_data(ii).y, '.', ...
        valid_lane_data(ii).left.x, valid_lane_data(ii).left.y, 'r', ...
...
% valid_lane_data = struct('left', struct('x', [], 'y', []), ...
%     'right', struct('x', [], 'y', []));
save('valid_lane_data.mat', 'valid_lane_data');