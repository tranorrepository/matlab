...
for ii = 1:num_of_valid_gps_tracks
    ...
    for jj = 1:num_of_Tcrossing
        ...
        tmp_ones = ones(2, num_of_pnts);
        tmp_ones(1, :) = tmp_ones(1, :) * center_cfg_pnts(jj, 1);
        tmp_ones(2, :) = tmp_ones(2, :) * center_cfg_pnts(jj, 2);
        
        tmp_dist = zeros(2, num_of_pnts);
        tmp_dist(1, :) = tmp_ones(1, :) - valid_gps_track_data(ii).x;
        tmp_dist(2, :) = tmp_ones(2, :) - valid_gps_track_data(ii).y;
        ...
        if ~isempty(valid_ind)
            ...
            for kk = 1 : length(jump_ind)-1
                ...
                gps_seg_group_data(jj).group(grp_ind(jj)).x = ...
                    valid_gps_track_data(ii).x(data_ind);
                gps_seg_group_data(jj).group(grp_ind(jj)).y = ...
                    valid_gps_track_data(ii).y(data_ind);
            end
        end
    end
